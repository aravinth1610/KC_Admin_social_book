...
public class KcUserController {
	...
	public final ResponseEntityWrapper<?>  createUser(@PathVariable(value="orgId") Long orgId ,@RequestBody User user) {
		System.out.println(user);
		return new ResponseEntityWrapper<>(userKcServices.createUser(orgId, user));
	}

	@GetMapping("/user/{id}")
	private final ResponseEntityWrapper<?> getUserById(@PathVariable(value = "id") String id) {
		System.out.println(id);
		return new ResponseEntityWrapper<>(userKcServices.getUserById(id));
	}

	@GetMapping("/user")
	private final ResponseEntityWrapper<?> getUser() {
		return new ResponseEntityWrapper<>(userKcServices.getListofUser());
	}

	@ResponseStatus(HttpStatus.NO_CONTENT)
	@DeleteMapping("/user/{id}")
	private final ResponseEntityWrapper<?> deleteUser(@PathVariable(value = "id") String id) {
		userKcServices.deleteUser(id);
		return new ResponseEntityWrapper<>();
	}

	@PutMapping("/user/{id}")
	public final ResponseEntityWrapper<?> updateUser(@PathVariable String id, @RequestBody User user) {
	...
	public final ResponseEntityWrapper<?> assignUserRole(@PathVariable(name="id") String id, @RequestParam(name="role") String role) {
		System.out.println(id+"-"+role);
		return new ResponseEntityWrapper<>(userKcServices.assignUserRoles(id, role));
	}
	
	@GetMapping("/user/{id}/role")
	private final ResponseEntityWrapper<?> getUser(@PathVariable("id") String id, @RequestParam("clientId") String clientId) {
